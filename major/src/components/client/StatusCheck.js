...
const StatusCheck = () => {
  ...
  return (
        ...
        {data.map((booking) => (
          cusername === booking.cusername && (
            <Card key={booking._id} style={{ width: "100%", boxShadow: "0px 0px 10px rgba(0, 0, 0, 0.1)", marginBottom: "20px" }}>
              <Card.Body>
                <Card.Title style={{ marginBottom: "10px", borderBottom: "1px solid #e2e2e2", paddingBottom: "10px" }}>Booking Details</Card.Title>
    ...
    <>
      <Card.Text style={{ fontWeight: "bold" }}>Payment</Card.Text>
      <Card.Text>RS: {booking.charges}</Card.Text>
      <Payment charges={booking.charges} />
    </>
  )}
